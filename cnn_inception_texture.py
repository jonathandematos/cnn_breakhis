...
from keras.layers import Dense, Dropout, Input, Flatten, Conv2D, MaxPooling2D, BatchNormalization, AveragePooling2D, Concatenate
...
def build_cnn(nr_convs):
    ...
    conv1_3_3 = Conv2D(32, (3, 3), strides=(1,1), name="conv1_3_3", activation='relu', padding='same')(input_layer)
    conv1_5_5 = Conv2D(32, (5, 5), strides=(1,1), name="conv1_5_5", activation='relu', padding='same')(input_layer)
    conv1_11_11 = Conv2D(32, (11, 11), strides=(1,1), name="conv1_11_11", activation='relu', padding='same')(input_layer)

    concat1 = keras.layers.concatenate([conv1_3_3, conv1_5_5, conv1_11_11])

    #model.add(Conv2D(64, (5, 5), strides=(1,1), name="conv1", activation='relu', input_shape=(HEIGHT,WIDTH,3)))
    #model.add(BatchNormalization(axis=3, name="batch1"))
    #model.add(MaxPooling2D(pool_size=(2,2), name="max1"))
    
    if(nr_convs > 1):
        conv2_3_3 = Conv2D(32, (3, 3), strides=(1,1), name="conv2_3_3", activation='relu', padding='same')(concat1)
        conv2_5_5 = Conv2D(32, (5, 5), strides=(1,1), name="conv2_5_5", activation='relu', padding='same')(concat1)
        conv2_11_11 = Conv2D(32, (11, 11), strides=(1,1), name="conv2_11_11", activation='relu', padding='same')(concat1)

        concat2 = keras.layers.concatenate([conv2_3_3, conv2_5_5, conv2_11_11], axis=3)

        #model.add(Conv2D(64, (5, 5), strides=(1,1), name="conv2", activation='relu'))
        #model.add(BatchNormalization(axis=3, name="batch2"))
        #model.add(AveragePooling2D(pool_size=(216,236), name="max2"))
        #model.add(Dropout(0.25))

    if(nr_convs > 2):
        model.add(Conv2D(128, (3, 3), name="conv3", activation='relu'))
        model.add(BatchNormalization(axis=3, name="batch3"))
        model.add(AveragePooling2D(pool_size=(220,340), name="max3"))
        #model.add(Dropout(0.25))
